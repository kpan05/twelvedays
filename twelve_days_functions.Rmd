...
make_phrase <- function(num, num_word, item, verb, adjective, location) {
  ...
  } else if (vowel_start == TRUE) {
  
      item <- str_replace(item, "\\w+", "an \\0")
    
  } else {
    
      item <- str_replace(item, "\\w+", "a \\0")
...
xmas$Full.Phrase <- str_replace(xmas$Full.Phrase, "one", "")   #get rid of the "1"
```
...
???
  ...
  glue({intro}, "my true love sent to me", "\\n", {phrases})
